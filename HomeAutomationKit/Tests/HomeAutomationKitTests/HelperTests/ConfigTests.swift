...
struct ConfigTests {
    ...
    func normalizeElevation() throws {
        ...
        let automation = MotionAtNight(noMotionWait: .milliseconds(50), motionSensors: [device1], lightSensor: device1, lights: [device2], windowContacts: [device3], minBrightness: 0.1)
        let config = Config(automations: [automation],
                            location: Location(latitude: 53.14194, longitude: 8.21292),
                            devices: [device1, device2, device3])
        let entityIds = Set(config.automations.flatMap(\.triggerEntityIds))
